...
def send_reminder_email(receiver):
    ...
    try:
        ...
        msg = EmailMessage()
        msg['Subject'] = "[EcoCAR] Reminder: Please Remember to Sign UPA !!!"
                ...
                <body style="background-color: #DBE1F1; width: 100%; font-family:Lato, sans-serif; font-size:18px;">
                    <div id="email">
                        ...
                        <table role="presentation" border="0" cellpadding="0" cellspacing="10px" style="padding: 30px 30px 30px 60px;">
                            <tr>
                                <td>
                                    <h2>Please sign the UPA DocuSign ASAP!!!</h2>
                                    <p>Our record shows that you haven't signed the UPA DocuSign.</p>
                                    <p>Please sign it ASAP to process your EcoCAR onboarding application.</p>
                                    <p>If you can't find the UPA DocuSign request email, please immediately contact Project Management Team (ecocar-pm@ucdavis.edu).</p>
...
with DAG(
    dag_id="DocuSign-Check_if_UPA_is_Signed",
    default_args=default_args,
    start_date=datetime(2023, 4, 10),
    schedule_interval="0 21 * * *",  # daily, see https://crontab.guru/ for more detailed formats.
    catchup=False,
) as dag:
    check_if_UPA_signed_task = PythonOperator(
        task_id="check_if_UPA_is_signed_task", 
        python_callable=check_if_UPA_is_signed,
        provide_context=True,  # Add this line to pass context to the function
    )

check_if_UPA_signed_task
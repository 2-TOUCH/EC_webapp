...
# function updating "isPhotoWaiverSigned" to be true after the PhotoWaiver DocuSign status has changed to be signed
def updateIsPhotoWaiverSigned(emailAddress):
    """
    This function updates the column "isPhotoWaiverSigned" according to the email address
    ...
    update_sql = """
        ...
        SET "isPhotoWaiverSigned" = true
        WHERE emailaddress = %s
    """
    # Check if the email even exists in the database
    cursor.execute(update_sql, (emailAddress,))
    
    print(f"Updated \"isPhotoWaiverSigned\" to be TRUE for \"{emailAddress}\" in EcocarAccessRequests.")
        
    # Commit the changes and close the connection
    conn.commit()
    conn.close()

# function inserting DocuSign ID into the "PhotoWaiverSentID" column in "EcocarAccessRequests"
def insertDocuSignSentID(envelopeID, emailAddress):
    ...
    Args:
        envelopeID (string): envelop ID of Photo Waiver doc
    ...
    insert_sql = """
        UPDATE "EcocarAccessRequests"
        SET "PhotoWaiverSentID" = %s
        WHERE "emailaddress" = %s
    """
    # Check if the email even exists in the database
    cursor.execute(insert_sql, (envelopeID, emailAddress))
    
    print(f"Inserted PhotoWaiverSentID: \"{envelopeID}\" for \"{emailAddress}\" in EcocarAccessRequests.")
...
def check_DocuSign_status(email, envelopeID):
    ...
    if status == 'completed':
        updateIsPhotoWaiverSigned(emailAddress=email)
...
def send_DocuSign_doc(emailAddress, firstName, lastName):
    ...
    template_id = os.getenv(
        "DOCUSIGN_PHOTO_WAIVER_TEMPLATE_ID"
    )
...
def get_approved_and_not_yet_sent_member_list():
    ...
    get_status_request = """
        SELECT emailaddress, "PhotoWaiverSentID"
        FROM "EcocarAccessRequests"
        WHERE teamleadapprove = 'APPROVED'
        AND itapprove = 'APPROVED'
        AND "isOnboardingSent" = true
...
with DAG(
    dag_id="DocuSign-Send_Photo_Waiver_Document",
    default_args=default_args,
    start_date=datetime(2023, 4, 10),
    schedule_interval="0 20 * * *",  # daily, see https://crontab.guru/ for more detailed formats.
    catchup=False,
) as dag:
    photo_waiver_docusign_task = PythonOperator(
        task_id="docusign_send_photo_waiver_task", 
        python_callable=get_approved_and_not_yet_sent_member_list
    )

photo_waiver_docusign_task